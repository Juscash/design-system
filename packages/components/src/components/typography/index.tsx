...
import type { ThemeConfig, GetProps } from 'antd';
import { designSystemColors } from '@ds/themes/colors';

// Tipos derivados conforme FAQ do Ant Design (evitar imports de subpaths)
type TitleProps = GetProps<typeof AntdTypography.Title>;
type TextProps = GetProps<typeof AntdTypography.Text>;
type ParagraphProps = GetProps<typeof AntdTypography.Paragraph>;
...
export type DSColor = keyof typeof colorMap;
...
const typographyVariants: Record<TypographyVariant, ThemeConfig> = {
  heading1: {
    components: {
      Typography: {
        fontSizeHeading1: 61,
...
export function Typography(props: CustomTypographyProps): React.ReactElement {
  ...
  const variantKey = (variant ?? 'body1') as TypographyVariant;
  const variantTheme = typographyVariants[variantKey];
  const colorKey = color as DSColor | undefined;
  const textColor = colorKey ? colorMap[colorKey] : undefined;
  ...
  return <ConfigProvider theme={{ components: variantTheme.components }}>{node}</ConfigProvider>;
}
